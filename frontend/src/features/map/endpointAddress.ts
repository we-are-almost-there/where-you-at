...
// Kakao coord2Address는 예전부터 있던 시도만 축약형("경남", "부산")으로 준다.
// 개편으로 생긴 이름(강원특별자치도, 전북특별자치도, 전남광주통합특별시)은 축약형 없이
// 정식 명칭 그대로 오므로, 목록에 없는 값은 원래 값을 통과시켜 화면이 비지 않게 방어한다.
//
// 전남·광주 항목을 두지 않는 이유: 두 지역은 전남광주통합특별시로 통합됐다. 통합 지역 좌표를
// 찍어보면 두 API 모두 정식 명칭만 주므로 축약형이 올 일도 없지만, 남겨두면 어쩌다 축약형이
// 왔을 때 폐지된 이름("전라남도", "광주광역시")을 출력하게 된다.
// (강원·전북은 축약형이 안 오더라도 매핑 결과가 현재 정식 명칭이라 그대로 둔다)
