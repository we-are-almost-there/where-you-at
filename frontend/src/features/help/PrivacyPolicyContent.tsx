 ...
 * - 완주 기록·기록 카드 항목(2번 ②), 보유 기간·파기(4·5번), 위탁·국외 이전(7·8번):
 *   backend/app/schemas/record.py, sql/11_run_record.sql, sql/12_record_card.sql,
 *   backend/app/services/storage.py(R2 업로드·회원별 객체 삭제). 회원 탈퇴 시 uploads/·avatars/·record-cards/
 *   객체를 정리한 뒤 회원 행을 삭제하고, run_record·record_card·user_sigungu_stamps는 FK 연쇄 삭제한다(README 참고).
 ...
 *   Cloudflare R2(프로필 사진·기록 카드 이미지).
 ...
 *   한 줄 소개는 이용자가 직접 적고 언제든 지우는 선택 항목이라 별도 동의 화면을 두지 않았다. 완주 기록과 기록 카드, 지역 스탬프도
    ...
    <div>
      ...
      <LegalSection section={S.purpose} level={level}>
        ...
        <Items ordered>
          <li>
            ...
            줄 소개·프로필 사진 표시와 수정, 완주 기록·기록 카드·지역 스탬프 모아 보기), 회원 탈퇴 처리
          </li>
          <li>
      ...
      <LegalSection section={S.items} level={level}>
        ...
        <Table
          ...
          rows={[
            ...
            [
              "마이페이지 프로필 (선택)",
              "한 줄 소개, 프로필 사진과 저장 객체 키 (이용자가 마이페이지에서 직접 입력·업로드한 경우에만)",
              "「개인정보 보호법」 제15조제1항제4호(계약의 체결·이행)",
            ],
            [
              "로그인 상태 유지 (필수)",
            ...
            [
              "지역 스탬프 (선택)",
              "회원 번호, 스탬프를 찍은 시군구 코드, 찍은 시각 (완주한 지역에서 이용자가 직접 찍기를 선택한 경우)",
              "「개인정보 보호법」 제15조제1항제4호(계약의 체결·이행)",
            ],
            [
              "완주 기록 및 기록 카드 (선택)",
              "따라간 코스, 경로 유형, 거리, 소요 시간, 페이스, 완주 여부, 종료 시각 (코스 따라가기를 마친 경우), 기록 카드 이미지 (이용자가 마이페이지에 저장하도록 선택한 경우에만)",
              "「개인정보 보호법」 제15조제1항제4호(계약의 체결·이행)",
            ],
          ]}
        />
        <P>
          ...
          가입할 때 카카오 닉네임으로 정해지고, 이후에는 마이페이지에서 바꿀 수 있습니다. 프로필 사진은 카카오에서 받는
          사진이 아니라 회원이 직접 고른 사진만 처리하며, 올리지 않거나 언제든지 기본 이미지로 되돌릴 수 있습니다. 한 줄
          소개와 프로필 사진은 선택 항목이며, 앞으로 쪽지·리뷰 기능에서 다른 이용자에게 보일 수 있습니다.
        </P>
      ...
      <LegalSection section={S.children} level={level}>
        ...
        <P>
          ...
          있습니다. 로그인할 때마다 만 14세 이상인지 확인하는 안내를 보여 드리며, 14세 미만 아동이 가입한 사실을 알게 되면
          해당 회원 정보를 지체 없이 파기합니다. 로그인하지 않고 이용하는 기능은 나이와 상관없이 이용할 수 있습니다.
        </P>
      </LegalSection>
      ...
      <LegalSection section={S.retention} level={level}>
        ...
        <Table
          ...
          rows={[
            [
              "회원 정보 (카카오 회원번호, 닉네임, 한 줄 소개, 프로필 사진과 저장 객체 키)",
              "회원 탈퇴 시까지(카카오에서 서비스와의 연결을 끊은 경우 포함). 탈퇴하면 지체 없이 파기합니다. 한 줄 소개는 이용자가 지우면 바로 삭제하고, 프로필 사진은 삭제하거나 새 사진으로 바꾸면 기존 파일과 저장 객체 키를 바로 삭제합니다.",
            ],
            [
            ...
            [
              "지역 스탬프 (회원 번호, 시군구 코드, 찍은 시각)",
              "회원 탈퇴 시까지. 탈퇴하거나 카카오에서 서비스 연결을 끊으면 회원 정보와 함께 데이터베이스에서 지체 없이 삭제합니다.",
            ],
            [
              "완주 기록 및 기록 카드",
              "회원 탈퇴 시까지. 탈퇴하면 완주 기록과 기록 카드 정보는 데이터베이스에서 삭제하고, 기록 카드 이미지는 비공개 이미지 저장소(R2)에서 함께 삭제합니다.",
            ],
            [
      ...
      <LegalSection section={S.destruction} level={level}>
        <Items ordered>
          ...
          <li>
            ...
            직접 삭제합니다. 프로필 사진은 회원이 삭제·교체할 때 기존 파일을 지우며, 회원 탈퇴 시 프로필 사진과 기록 카드
            이미지를 비공개 이미지 저장소에서 삭제하고 완주 기록·기록 카드·지역 스탬프 정보도 데이터베이스에서 함께 삭제합니다.
            로그인 세션은 로그아웃하면 현재 세션을 삭제하고, 회원을 탈퇴하면 모든 세션을 회원 정보와 함께 삭제합니다.
            만료된 세션은 같은 회원이 다음에 로그인할 때 삭제합니다. 보유 기간이 지난 1:1 문의는 데이터베이스의 예약 작업이
            매일 자동으로 찾아 파기합니다. 삭제 또는 동의 철회 요청을 확인하거나 14세 미만 아동의 문의임을 알게 되면,
            운영팀은 관련 Slack 알림과 데이터베이스의 문의를 직접 찾아 지체 없이 삭제합니다. 서버 접속 기록은 각 업체가
            정한 로그 보관 기간(4번)에 따라 처리됩니다.
          </li>
          {/* "복구할 수 없도록 삭제"는 수탁자 시스템에 남을 수 있는 사본까지 운영팀이 보장할 수 없어 쓰지 않는다.
            ...
            이 항목에 백업 보관 기간을 적는다. */}
          <li>
            <strong>파기 방법</strong>: 전자 파일 형태로 저장된 개인정보는 데이터베이스와 비공개 이미지 저장소에서 삭제해
            운영팀이 다시 조회하거나 복원할 수 없게 합니다. 운영팀은 개인정보가 담긴 데이터베이스나 프로필 사진·기록 카드
            이미지를 따로 내려받아 백업해 두지 않으며, 종이 문서로도 보관하지 않습니다. 새 문의 알림에 포함된 개인정보는
            Slack 메시지를 직접 삭제하며, 별도 요청이 없으면 Slack 워크스페이스의 90일 보존 설정에 따라 자동으로
            삭제됩니다. 그 밖에 수탁자(7번)가 시스템 운영 과정에서 보관하는 사본이 있는 경우, 그 사본은 수탁자의 정책에
            따라 삭제됩니다.
          </li>
        </Items>
      </LegalSection>
      ...
      <LegalSection section={S.outsourcing} level={level}>
        ...
        <Table
          ...
          rows={[
            [
              "Supabase, Inc.",
              "회원 정보, 완주 기록·기록 카드·지역 스탬프 정보와 1:1 문의 정보를 보관하는 데이터베이스(클라우드) 운영",
            ],
            ["Vercel Inc.", "웹사이트 호스팅 (웹사이트 전송 과정에서 서버 접속 기록 처리)"],
            [
              "Render Services, Inc.",
              "API 서버 운영 (회원 정보, 완주 기록·기록 카드·지역 스탬프 정보, 1:1 문의 정보, 서버 접속 기록 처리)",
            ],
            [
              "Cloudflare, Inc.",
              "회원이 올린 프로필 사진을 비공개 객체 저장소(R2)에 저장·전송하고 삭제하며, 기록 카드 이미지도 같은 저장소에 저장·전송하고 삭제",
            ],
      ...
      <LegalSection section={S.overseas} level={level}>
        {/* Vercel·Render·Supabase는 미국 법인이고 Render에는 서울 리전이 없어(2026.9. 기준 오리건·오하이오·버지니아·
          ...
          국외 이전 사실을 한 줄로 알린다(ContactPage). Cloudflare(R2)는 프로필 사진과 기록 카드 이미지의
          저장 위치·이전 항목·보유 기간을 함께 적는다. */}
        <P>
        ...
        <Table
          ...
          rows={[
            ...
            [
              ...
              "싱가포르",
              "회원 정보(카카오 회원번호, 닉네임, 한 줄 소개, 프로필 사진 저장 객체 키), 완주 기록·기록 카드·지역 스탬프 정보(코스, 경로 유형, 거리, 소요 시간, 페이스, 완주 여부, 종료 시각, 기록 카드 저장 객체 키, 스탬프 시군구 코드와 찍은 시각), 1:1 문의 정보(이메일, 문의 유형, 문의 내용), 서버 접속 기록(접속 IP 주소, 요청 일시, 요청 주소)",
              "서비스를 이용할 때 네트워크를 통해 전송",
              "API 서버 운영",
              "회원 정보, 완주 기록·기록 카드·지역 스탬프 정보와 1:1 문의 정보는 요청을 처리한 뒤 저장하지 않음. 로그 보관 기간 7일 (Hobby 요금제 기준)",
            ],
            [
              ...
              "회원 정보(카카오 회원번호, 닉네임, 한 줄 소개, 프로필 사진 저장 객체 키, 가입·수정 일시), 완주 기록·기록 카드·지역 스탬프 정보(코스, 경로 유형, 거리, 소요 시간, 페이스, 완주 여부, 종료 시각, 기록 카드 저장 객체 키, 스탬프 시군구 코드와 찍은 시각), 1:1 문의 정보(이메일, 문의 유형, 문의 내용, 접수·동의·처리 일시)",
              "데이터베이스 운영과 장애 대응 등 기술 지원이 필요할 때 원격으로 접근",
              "데이터베이스(클라우드) 운영과 기술 지원",
              "회원 정보와 완주 기록·기록 카드·지역 스탬프 정보는 회원 탈퇴 시까지, 1:1 문의는 문의 처리 완료 후 1년 (4번과 같음)",
            ],
            [
              // Cloudflare는 미국 법인이고 운영 R2 버킷 위치는 APAC이다. 버킷 위치·관할을 바꾸면 이전 국가도
              // Cloudflare 대시보드의 Bucket details와 맞춰 다시 확인한다.
              "Cloudflare, Inc. (legal@cloudflare.com)",
              "미국 (처리 주체), 아시아·태평양(APAC) (객체 저장 위치)",
              "회원이 직접 올린 프로필 사진과 저장 객체 키, 마이페이지에 저장한 기록 카드 이미지와 저장 객체 키",
              "프로필 사진을 등록·변경·조회하거나 기록 카드 이미지를 저장·조회할 때 암호화된 네트워크를 통해 전송",
              "비공개 프로필 사진 및 기록 카드 이미지 저장·전송과 삭제",
              "프로필 사진 삭제·교체 또는 회원 탈퇴 시까지, 기록 카드 이미지는 회원 탈퇴 시까지",
            ],
            [
        ...
        <P>
          국외 이전을 원하지 않으면 회원 가입과 1:1 문의를 이용하지 않을 수 있습니다. Cloudflare로의 이전을 원하지 않으면
          프로필 사진을 올리지 않고 기본 이미지로 이용하고, 기록 카드를 마이페이지에 저장하지 않을 수 있습니다. 다만
          웹사이트와 API 서버가 위 업체를 통해 운영되므로 Vercel·Render로의 이전을 거부하면 서비스를 이용할 수 없습니다.
        </P>
      </LegalSection>
        ...
        <Items ordered>
          ...
          <li>
            {/* HTTPS: Vercel·Render는 배포한 주소에 HTTPS 인증서를 자동으로 적용한다. 프론트가 API를 부를 때도
              ...
              기록 카드 이미지는 서버에서 검증한 뒤 R2에 저장하고, 조회에는 만료 시간이 있는 presigned URL을 쓴다. */}
            <strong>기술적 조치</strong>: 데이터베이스 접속 정보를 코드와 분리해 관리, 데이터베이스 행 수준 보안(RLS)으로
            외부에 공개된 접근 경로 차단, 프로필 사진 비공개 저장과 만료되는 조회 주소 사용, 업로드 파일의 소유자·형식·용량
            검증, 기록 카드 이미지의 서버 검증 후 비공개 저장과 만료되는 조회 주소 사용, 반복 전송 제한, 보유 기간이 지난
            개인정보 자동 파기, 암호화된 통신(HTTPS)으로 전송
          </li>
          <li>
      ...
      <LegalSection section={S.autoCollect} level={level}>
        ...
        <P>
          ...
          저장합니다. 토큰에는 회원 번호, 로그인 세션 번호와 발급·만료 시각이 담기며 7일 뒤 만료됩니다. 로그아웃하거나
          브라우저에서 사이트 데이터를 삭제하면 지워집니다.
        </P>
        <P>
      ...
      <LegalSection section={S.rights} level={level}>
        <Items ordered>
          ...
          <li>
            회원은 마이페이지에서 닉네임과 한 줄 소개를 직접 확인하고 고칠 수 있으며, 프로필 사진을 직접 바꾸거나 삭제할
            수 있고, 완주 기록과 기록 카드, 직접 찍은 지역 스탬프를 확인할 수 있습니다. 회원 탈퇴로 회원 정보와 마이페이지에 저장한 기록 전체의
            삭제를 요구할 수도 있습니다.
          </li>
          <li>
