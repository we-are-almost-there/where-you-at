...
카카오는 3초 안에 `200`을 기대하고 재시도 정책은 문서에 없습니다. 그래서 삭제가 실패해도 `200`을 돌려주고
`[ERROR] 연결 끊기 웹훅 회원 삭제 실패(직접 삭제 필요): kakao_id=...` 로그를 남깁니다. 이 로그를 보면 해당
`kakao_id`의 `app_user` 행을 Supabase에서 직접 지웁니다.
