...
- 운영팀 채널로 `[어디까지왔니] 기록 카드 커밋 확인 실패 — 수동 확인 필요` Slack 알림. 회원 번호와
  이미지 키를 함께 보냅니다(같은 `INQUIRY_WEBHOOK_URL` 채널).

알림을 받으면 다음을 확인합니다.

1. Supabase `record_card` 테이블에서 알림에 적힌 이미지 키(`image_key`)로 행이 있는지 찾습니다.
...
Render 로그도 함께 확인합니다. `[ERROR] 기록 카드 커밋 여부 확인 실패(이미지 유지, Slack 알림): <image_key> <오류>`
줄에서 원래 어떤 오류로 재확인까지 실패했는지 볼 수 있습니다. Render 로그는 7일만 보관되므로, 알림을 받고도
