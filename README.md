...
새 문의 알림은 아직 없습니다. Slack 알림은 개인정보처리방침 7·8번에 수탁자를 추가하는 후속 이슈
[#103](https://github.com/we-are-almost-there/where-you-at/issues/103)에서 붙입니다.
그 전까지 확인 당번이 **매주 월요일·목요일** 아래 두 곳을 확인합니다.

- Supabase 테이블 편집기의 `inquiry` 테이블에서 `status`가 `완료`가 아닌 문의(`접수`·`처리중`). `처리중`으로 바꿔 두고
  답장을 잊은 문의도 여기서 다시 보입니다.
...
- 당번은 PR 설명과 팀 채널에 적어 둡니다. 알림이 붙으면 이 항목을 알림 기준으로 고칩니다.
