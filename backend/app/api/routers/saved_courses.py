...
def list_saved_courses(current_user: CurrentUser = Depends(get_current_user)):
    """찜한 코스 목록. 최근 찜한 순이며 코스 카드에 필요한 값이 모두 담긴다."""
    with db_connection() as conn:
        return {"courses": crud.list_saved(conn, current_user.id)}


@router.get("/keys", response_model=list[SavedCourseKey])
def list_saved_course_keys(current_user: CurrentUser = Depends(get_current_user)):
    """찜한 코스와 종목만. 목록·상세의 하트 상태를 그리는 데 쓴다.

    카드용 목록과 나눈 이유: 하트만 그리면 되는 화면이 썸네일 좌표(코스당 약 40점)까지 받지 않게 한다.
    """
    with db_connection() as conn:
        return crud.list_keys(conn, current_user.id)


@router.post("", status_code=201)
def add_saved_course(body: SavedCourseCreate, current_user: CurrentUser = Depends(get_current_user)):
    ...
    with db_connection() as conn:
        try:
            crud.add(conn, user_id=current_user.id, course_id=body.course_id, route_type=body.route_type)
...
def remove_saved_course(
    ...
    with db_connection() as conn:
        crud.remove(conn, user_id=current_user.id, course_id=course_id, route_type=route_type)
